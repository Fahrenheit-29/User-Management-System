...
//			
			...
			if(operation.checkInput(first_name)|| operation.checkInput(last_name)||operation.checkInput(email)
			...
			else if (!operation.pattern(operation.owasp_email, email)) {   	//email pattern ""
				...
				if (!operation.checkInput(file)) {				//check if the file is not null;
					
					if(!file.equals("default.png") && !oldfile.equals("default.png")) {			//if file is not equals to default.png & old file is not default.png
																								// if oldpath is not equals to  default.png process yung pagpalit ng file na buburain yung old file
						
					
					User data=new User(id,first_name,last_name,mi,email,address,phone,job,file);
					int update=execution.updateProfile(data);
					
					if (update!=0) {
						aws.deleteObject(bucketName,oldfile);
						session.setAttribute("userprofile", data);
						request.getSession().setAttribute("msg_edit", "Update Successful");
					...
					User previousprofile=(User)session.getAttribute("userprofile");
					User data=new User(id,first_name,last_name,mi,email,address,phone,job,previousprofile.getFile());
					int update=execution.updateProfile(data);
					
					if (update!=0) {
						
						session.setAttribute("userprofile", data);
						request.getSession().setAttribute("msg_edit", "<script> alert('Update Succesful') </script>");
						response.sendRedirect("UserPage");
